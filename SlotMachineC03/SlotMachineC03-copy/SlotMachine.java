...
public class SlotMachine {
    ...
    public void swap(int wheel1, int wheel2) {
        ...
        // Se valida que ambas ruedas existan dentro del rango actual [1, wheels.size()]
        if (wheel1 < 1 || wheel1 > wheels.size() || wheel2 < 1 || wheel2 > wheels.size()) {
            handleError("Posición de rueda inválida.");
            return;
        }
    ...
    /**
     * Crea una máquina tragamonedas con n ruedas y n símbolos inicializada aleatoriamente.
     * 
    ...
    public SlotMachine(int n) {
        ...
        if (n <= 0) {
            handleError("El número de ruedas y símbolos debe ser mayor a 0.");
            return;
        }

        String[] colorsPalette = {
            "red", "blue", "yellow", "green", "magenta", 
            "cyan", "orange", "pink", "black", "gray"
        };
        ...
        for (int i = 1; i <= n; i++) {
            String color = colorsPalette[(i - 1) % colorsPalette.length];
        ...
        for (int i = 1; i <= n; i++) {
            int randomSteps = random.nextInt(n) + 1;
            spin(i, randomSteps);
        }
    }
}